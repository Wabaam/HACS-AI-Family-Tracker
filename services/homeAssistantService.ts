import { User, Zone, TripData, HassEntity, ZoneEvent } from '../types.ts';
import { resetAiClient } from './geminiService.ts';
...
export const saveConfiguration = (config: { apiKey: string; entityIds: string[] }) => {
  localStorage.setItem(CONFIG_STORAGE_KEY, JSON.stringify(config));
};

export const loadConfiguration = (): { apiKey: string; entityIds: string[] } | null => {
  const storedConfig = localStorage.getItem(CONFIG_STORAGE_KEY);
  if (!storedConfig) return null;
  try {
    const config = JSON.parse(storedConfig);
    // Basic validation
    if (typeof config.apiKey === 'string' && Array.isArray(config.entityIds)) {
      return config;
    }
    return null;
  } catch (e) {
...
export const updateApiKey = (apiKey: string) => {
    ...
    if (config) {
        ...
        // After updating the key, we must reset the AI client so it re-initializes with the new key.
        resetAiClient();
    }
};
