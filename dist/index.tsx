...
(() => {
  ...
  // FIX: Cast window properties to `any` to resolve typing errors for globals.
  const { useState, useEffect, useCallback, useMemo, StrictMode, Fragment } = (window as any).React;
  const { createRoot } = (window as any).ReactDOM;
  const { MapContainer, TileLayer, Marker, Popup, Circle, useMap, useMapEvents } = (window as any).ReactLeaflet;
  const { Settings, Users, CheckCircle, KeyRound, LogIn, LogOut, History: HistoryIcon, Clock, PlusCircle, X, Check, MapPin, Trash2, Loader, Battery, Wind, BarChart, AlertTriangle, ShieldCheck, Landmark } = (window as any).lucide;
  const { GoogleGenAI } = (window as any).google.generativeai;
  ...
  const ZonePanel = ({ zone, events }) => {
    ...
    // FIX: Use .getTime() for correct date arithmetic.
    const findDurationForExit = (exitEv, allEvs) => { if (exitEv.type !== 'exit') return null; const entry = allEvs.find((e) => e.userId === exitEv.userId && e.type === 'entry' && e.timestamp < exitEv.timestamp); return entry ? formatDuration(exitEv.timestamp.getTime() - entry.timestamp.getTime()) : null; };
  ...
  const MapComponent = ({ users, zones, onUserSelect, selectedUser, selectedZone, zoneEditorMode, onMapClickForZone, onSelectZoneForEdit, editingZonePreview, onEditingZoneLocationChange }) => {
    // FIX: Cast prototype to `any` to delete property for leaflet icon fix.
    delete ((window as any).L.Icon.Default.prototype as any)._getIconUrl;
    (window as any).L.Icon.Default.mergeOptions({ iconRetinaUrl: 'https://unpkg.com/leaflet@1.9.4/dist/images/marker-icon-2x.png', iconUrl: 'https://unpkg.com/leaflet@1.9.4/dist/images/marker-icon.png', shadowUrl: 'https://unpkg.com/leaflet@1.9.4/dist/images/marker-shadow.png' });
    const createAvatarIcon = (url, sel) => (window as any).L.divIcon({ html: `<img src="${url}" style="width: 40px; height: 40px; border-radius: 50%; ${sel ? 'border: 3px solid #3b82f6;' : 'border: 2px solid #6b7280;'} box-shadow: 0 2px 5px rgba(0,0,0,0.5);" />`, className: '', iconSize: [40, 40], iconAnchor: [20, 20], popupAnchor: [0, -20] });
  ...
  const App = () => {
    ...
    const handleSelectZoneForEdit = useCallback((zone) => { setSelectedUserId(null); setSelectedZoneId(null); setZoneEditorMode('edit'); setEditingZone({ id: zone.id, name: zone.name, radius: zone.radius, location: (window as any).L.latLng(zone.location.lat, zone.location.lng) }); }, []);
